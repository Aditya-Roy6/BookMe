# BookMe - High-Concurrency Ticket Booking & Waitlist Engine
...
- **Real-Time Synchronization:** Server-Sent Events (SSE) via `text/event-stream`
- **Authentication:** JWT with Role-Based Access Control (`customer`, `organiser`, `admin`), bcryptjs password hashing
...
| Method | Endpoint | Access | Description |
|---|---|---|---|
| `POST` | `/api/venues` | Admin | Create physical venue layout (`totalRows` x `totalCols`) |
...
- When a user selects seats and clicks **Review and Pay**, the request hits `/api/showtimes/:id/hold`.
...
- If even one seat is contended by another customer, the script returns `0` and the API immediately aborts with `409 Conflict`, guaranteeing zero double-holding or partial cart reservations.
